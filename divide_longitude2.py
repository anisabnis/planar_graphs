...
r_longs = [x[0] for x in router_positions]
r_longs.sort()
ml = r_longs[int(len(r_longs)/2)] - 2
print("median longitude : ", r_longs[int(len(r_longs)/2)] - 2)
...
print(s_positions)
lats = [x[0] for x in s_positions]
index = list(range(len(s_positions)))
index.sort(key = lats.__getitem__)

print("here ", len([x for x in lats if x >= ml]))
print(user_servers)
print(index)
...
def create_sub_topology(hosts, G):
    ...
    for p in paths:
        ...
        for i, r in enumerate(p[1:len(p)-1]):
            ...
            if i == len(p) - 3:
                outdegree[r][p[i+2]] = outdegree[r][p[i+2]] + 1 + 1 + 1
                print("outdegree at " +  str(r))

            indegree[r][p[i]] = indegree[r][p[i]] + 1
            outdegree[r][p[i+2]] = outdegree[r][p[i+2]] + 1

    two_degree_routers = [r for r in indegree if len(indegree[r]) <= 2 and len(outdegree[r]) <= 2 and r not in egress]
...
server_cluster = dict()
print(sortedservers)
server_cluster[0] = sortedservers[:4]
server_cluster[1] = sortedservers[4:]
server_cluster[3] = sortedservers[3:7]
