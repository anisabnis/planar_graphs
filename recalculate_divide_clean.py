...
class ConstraintBuilder():
    ...
    def build_adjacency(self):
        ...
        new_vertices = set()
        new_vertices.update(['p1', 'p2', 'p3', 'p4'])
                            

        self.a = defaultdict(set)

        f = open(self.path + '/req_servers.txt', 'r')

        for l in f:
            l = l.strip().split(' ')
            req_servers.update(l)
        ...
        for i in [1,2]:
            ...
            for l in f:
                ...
                ## Todo : Build set of vertices

                for ii in range(len(l) - 1):
                    all_edges.add(((l[ii],l[ii+1])))
                
                if l[0] in req_servers:
                    for ii in range(2):
                        e=((l[ii], l[ii+1]))
                        ignore_edges.add(e)

                if l[-1] in req_servers:
                    for ii in range(len(l)-2, len(l)-1):
                        e = ((l[ii], l[ii+1]))
                        ignore_edges.add(e)

            print("Ignore edges : ", ignore_edges, len(ignore_edges))
            ...
            for l in f:
                ...
                for ii in range(len(l)-1):
                    ...
                    if e in ignore_edges:                    
                        ...
                        s_var = self.vh.getVariable('s', 's_' + str(l[ii]) + '_' + str(l[ii+1]) + '_' + str(l[0]))
                        c = Constraint(1, 1, [[s_var]], [1])
                        #self.constraints[c.id] = c

                        self.a[l[ii]].add(l[ii+1])
                        ...
                        ### To do : See if you can fix non edges to 0
                        edges_by_src[l[0]].add(((l[ii], l[ii+1])))
        ...
        print("v1  : ", self.v1)
        print("v2 : ", self.v2)
        print("v1 union v2 : ", self.v1.union(self.v2))
        
        print("v3 : ", self.v3)
        print("v4 : ", self.v4)
        print("v3 union v4 : ", self.v3.union(self.v4))

        

        for v in self.v1 - self.v2:
            self.a[v] = self.v1.union(self.v2)
            
        for v in self.v2 :
            self.a[v] = ((self.v2.union(self.v1)).union(self.v3))

        for v in self.v4 - self.v3:
            self.a[v] = (self.v4).union(self.v3)

        for v in self.v3:
            self.a[v] = (self.v3.union(self.v2)).union(self.v4)

        for key in self.a:
            print(key, self.a[key])

        self.v = self.v1.union(self.v2)
        self.v.union(self.v3)
        self.v.union(self.v4)
        self.v.update(new_vertices)                                                                        
